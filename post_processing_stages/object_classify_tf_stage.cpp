...
void ObjectClassifyTfStage::initialize()
{
	...
	const std::vector<int> inputs_ = interpreter_->inputs();
	const std::vector<int> outputs_ = interpreter_->outputs();

	if (interpreter_->AllocateTensors() != kTfLiteOk)
		throw std::runtime_error("Failed to allocate tensors");

	if (readLabelsFile(settings_.labels_file_name, &labels_, &label_count_) != kTfLiteOk)
		throw std::runtime_error("Failed to read labels file");
}
...
void ObjectClassifyTfStage::Configure()
{
	...
	if (stream_)
	{
		...
		if (w_ != IMAGE_WIDTH || h_ != IMAGE_HEIGHT)
			throw std::runtime_error("Lores width and height must be 224");
	}
}
...
bool ObjectClassifyTfStage::Process(CompletedRequest &completed_request)
{
	...
	std::string annotation = "Detected";

	for (const auto &result : output_results_)
	{
		annotation = annotation + ", " + result.first + " : " + std::to_string(result.second);
	}

	completed_request.post_process_metadata.Set("object_classify.results", output_results_);

	if (display_labels_ == 1)
		completed_request.post_process_metadata.Set("annotate.text", annotation);
...
{
	int input_ = interpreter_->inputs()[0];
	settings_.input_type = interpreter_->tensor(input_)->type;

	for (int i = 0; i < tensor_input_.size(); i++)
	{
		interpreter_->typed_tensor<uint8_t>(input_)[i] = static_cast<uint8_t>(tensor_input_[i]);
	}
	...
	getTopResults(interpreter_->typed_output_tensor<uint8_t>(0), output_size, top_n_results_, threshold_high_,
				  threshold_low_, &top_results_, settings_.input_type);
...
{
	uint8_t *Y = mem, *U = mem + h * stride, *V = mem + (5 / 4) * h * stride;
	int dst;
	...
	for (int y = 0; y < h; y++)
	{
		...
		{
			Y = mem + y * stride + x;
			U = mem + (h * stride) + ((int)floor(y / 2)) * (stride / 2) + ((int)floor(x / 2));
			V = mem + (5 * h * stride) / 4 + ((int)floor(y / 2)) * (stride / 2) + ((int)floor(x / 2));
			dst = (x + y * w) * 3;
